...
WITH 
    ...
    (
		...
		SELECT
            ...
            ROUND((total_exports_usd - LAG(total_exports_usd) OVER
				(PARTITION BY reporter_iso3 ORDER BY reference_year)) / NULLIF(LAG(total_exports_usd) OVER
					(PARTITION BY reporter_iso3 ORDER BY reference_year), 0 ) * 100, 2 ) 					AS yoy_export_growth_pct
		FROM 
			dashboard_data;
            

-- NOTE: Below View added later on while creating MS Excel workbook.
-- Creating view: vw_scenario_analysis_dataset
-- This has been created for using this dataset in MS Excel for creating Scenario Analysis dashboard. This will provide business metrics that will be used for What-If Analysis.
DROP VIEW IF EXISTS vw_scenario_analysis_dataset;
CREATE VIEW 
...
WITH 
    ...
    (
		...
		SELECT
            ...
            gdp_growth_annual_pct,
            trade_pct_of_gdp,
            ROUND( total_exports_usd / NULLIF(population_total,0), 2 ) 								AS export_per_capita,
            ROUND( gdp_current_usd / NULLIF(population_total,0), 2 ) 								AS gdp_per_capita,
            ROUND( (total_exports_usd / NULLIF(gdp_current_usd,0))*100, 2 ) 						AS export_to_gdp_ratio_pct,
            RANK() OVER ( PARTITION BY reference_year ORDER BY total_exports_usd DESC ) 			AS global_export_rank,
            LAG(total_exports_usd) OVER ( PARTITION BY reporter_iso3 ORDER BY reference_year ) 		AS previous_year_exports,
            ROUND( (total_exports_usd - LAG(total_exports_usd) OVER 
				( PARTITION BY reporter_iso3 ORDER BY reference_year)) / 
					NULLIF(LAG(total_exports_usd) OVER (
						PARTITION BY reporter_iso3 ORDER BY reference_year), 0)*100, 2) 			AS yoy_export_growth_pct
		FROM 
			scenario_base;